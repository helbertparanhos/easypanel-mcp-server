...
### Fixed (revisão de código pré-release)
- **Leituras com parâmetro numérico eram inalcançáveis pelo `easypanel_raw`.** As 5 operações de métricas/logs (`getMetricsSystemStats`, `getMetricsServiceStats`, `getAllServicesStats`, `queryServiceLogs`, `queryComposeServiceLogs`) declaram `range`/`limit`/`levels` não-string, então a query string as recusa; e como não estavam na tabela de tradução, o desvio para o transporte interno também falhava — com uma mensagem que afirmava (falsamente) que a operação não existia no painel. Os 5 nomes internos foram confirmados ao vivo e mapeados; um teste agora falha se qualquer leitura de param não-string ficar sem rota.
- **Uma falha transitória na sonda de detecção prendia o processo na API interna.** Um 502 do proxy ou um soluço de rede em `GET /api/getUpdateStatus` fazia a detecção cair nas rotas antigas — que seguem vivas no 2.33 — e cachear `rpc` para sempre. A sonda agora separa "o painel respondeu que a rota não existe" (conclusivo) de "não deu para saber" (rede/5xx); no segundo caso o resultado **não é cacheado** e a próxima chamada tenta de novo.
- **Nome achatado não funcionava em painéis ≤ 2.32.** A doc da v3 orienta usar `listCertificates`, mas as gerações antigas só entendem `certificates.listCertificates` — o nome achatado virava `/api/rpc/listCertificates` (404). Agora é traduzido nos dois transportes antigos.
- **`null` de um 200 sem corpo podia virar `TypeError`** em `get_env_vars`/`set_env_var`/`delete_env_var` e em `inspect_database`, no lugar de um erro limpo.
- **A orientação de ligar/desligar banco citava um nome que não existe em painéis antigos.** Agora é condicional à geração: nome da API pública no 2.33+, botão do painel nos anteriores.
